...
function Board() {
  ...
  const checkerCollection = collection(db, 'checkers')
  const [dataId] = useCollection(checkerCollection, {idField: 'id'})
            ...
            new Array(8).fill(0).map((column,columnKey)=>{
              return <div key={columnKey} className="squares">
                {new Array(8).fill(0).map((row,rowKey)=>{
                  ...
                  const y = rowKey
                  const x = columnKey 
                  const pawnImg1 = <img draggable="true" onDragStart={()=>{onSelect(idSrt)}} src={king.includes(idSrt) ? checkerPieceKing1 : checkerPiece1} style={{width : '60px'}} alt="" />
                  const pawnImg2 = <img draggable="true" onDragStart={()=>{onSelect(idSrt)}} src={king.includes(idSrt) ? checkerPieceKing2 : checkerPiece2} style={{width : '60px'}} alt="" />
                  return (
                    <div 
                      key={rowKey}
                      ...
                      id={idSrt}
                      className='square' 
                      style={((id) % 2 === 0) ? {backgroundColor:'rgb(163, 162, 162)', color: "white", cursor: 'pointer',}: {backgroundColor:'white'}} >
